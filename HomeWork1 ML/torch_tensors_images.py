...
def show_tensor_image(img_tensor: torch.Tensor, title: str, save_path: Path | None = None) -> None:
    ...
    img_numpy = img_tensor.permute(1, 2, 0).detach().cpu().numpy()
...
def main() -> None:
    ...
    print(f"Tensor min={img_tensor.min().item():.4f}, max={img_tensor.max().item():.4f}")
    ...
    reshaped = vec.reshape(img_tensor.shape)
    assert torch.allclose(reshaped, img_tensor), "Reshaped tensor does not match original!"
